...
const defaultSearchResults = 1_000
const defaultSearchWalkDepth = 64
...
func Walk(ctx context.Context, sources []Source, opt WalkOptions) (WalkResult, error) {
	...
	if opt.Limit <= 0 && !opt.Unbounded {
		opt.Limit = defaultSearchResults
	}
...
func (w *walker) visit(j job) {
	...
	if j.depth > defaultSearchWalkDepth {
		w.mu.Lock()
		w.partial = true
		w.mu.Unlock()
		return
	...
	rd := src.Reader
	err := rd.ReadDir(j.path, func(e Entry) bool {
		seen++
		child := joinPath(j.path, e.Name)
		isDir := e.Kind.IsDir()
		if !src.Allows(child, isDir) {
			return true
		}
		if isDir {
			children = append(children, job{src: j.src, path: child, depth: j.depth + 1})
		}
		if matchesName(e.Name, w.opt.Needle) && w.opt.Filter.Admits(e.Name, isDir) {
			matched = append(matched, pending{
				...
				path: child, name: e.Name, isDir: isDir,
			})
			entSeq++
		}
		return true
...
func statAll(sources []Source, batch []pending) {
	...
	for i := range batch {
		p := &batch[i]
		st, err := sources[p.src].Reader.Stat(p.path)
		if err != nil {
			continue
		}
		size := st.Size
...
func (w *walker) hitsOf(batch []pending) []Hit {
	...
	for _, p := range batch {
		src := w.sources[p.src]
		path := joinDisplay(src.Prefix, p.path)
		hits = append(hits, Hit{Share: uint32(src.Namespace), Path: path, Name: p.name,
			IsDir: p.isDir, Size: p.statSize, MTimeNs: p.statMTime,
			Score: Score(RankInput{NameFolded: FoldString(p.name), Needle: w.opt.Needle,
				Path: path, MTimeNs: p.statMTime, NowNs: w.opt.NowNs, Scope: w.opt.Scope})})
	}
	return hits
}
