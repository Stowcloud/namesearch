...
const defaultCorpusScanEntries int64 = 5_000_000
...
func ScanCorpus(ctx context.Context, sources []Source, opt ScanOptions) (ScanResult, error) {
	...
	if limit <= 0 {
		limit = defaultCorpusScanEntries
	}
	...
	for _, src := range sources {
		...
		for len(stack) > 0 {
			...
			rd := src.Reader
			rerr := rd.ReadDir(dir, func(e Entry) bool { entries = append(entries, e); return true })
			if rerr != nil {
				continue
			}
			out.DirsVisited++
			for _, e := range entries {
				...
				Trigrams(folded, func(t Trigram) { b := t.Bytes(); sketch.AddHash(Hash64(b[:])) })
			}
		}
	}
	return finishScan(out, sketch), nil
