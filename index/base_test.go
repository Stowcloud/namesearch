...
func entries(n int) []Entry {
	out := make([]Entry, 0, n)
	for i := range n {
		out = append(out, Entry{Namespace: 1, Path: fmt.Sprintf("dir%02d/file-%04d.txt", i%8, i)})
	}
	TreeOrder(out)
	return out
}
...
func TestBlockPayloadGoldenEncoding(t *testing.T) {
	in := []Entry{{Namespace: 1, Path: "a.txt"}, {Namespace: 300, Path: "b"}}
	raw := EncodeBlock(in)
...
func TestHighDFPruningDropsTheListAndKeepsTheEntry(t *testing.T) {
	...
	for i := range 32 * MinBlocksForPrune {
		corpus = append(corpus, Entry{Namespace: 1, Path: fmt.Sprintf("common-%04d.txt", i)})
	}
	TreeOrder(corpus)
	seg := mustWriteBase(t, corpus, 32, 0.6)
...
func TestEachUnderVisitsOneSubtreeOnly(t *testing.T) {
	corpus := []Entry{
		{Namespace: 1, Path: "a/one.txt"},
		{Namespace: 1, Path: "a/two.txt"},
		{Namespace: 1, Path: "a/deep/three.txt"},
		{Namespace: 1, Path: "b/four.txt"},
		{Namespace: 2, Path: "a/five.txt"},
	}
	TreeOrder(corpus)
...
// The namespace root, named by the empty string, holds everything in that namespace
// and nothing from another.
func TestEachUnderAtTheShareRoot(t *testing.T) {
	corpus := []Entry{
		{Namespace: 1, Path: "a.txt"},
		{Namespace: 1, Path: "d/b.txt"},
		{Namespace: 2, Path: "c.txt"},
	}
	TreeOrder(corpus)
	seg := mustWriteBase(t, corpus, 2, 0.6)
...
func TestTreeOrderSortsByNamespaceThenPath(t *testing.T) {
	in := []Entry{
		{Namespace: 2, Path: "a"},
		{Namespace: 1, Path: "b"},
		{Namespace: 1, Path: "a"},
	...
	TreeOrder(in)
	want := []Entry{{Namespace: 1, Path: "a"}, {Namespace: 1, Path: "b"}, {Namespace: 2, Path: "a"}}
...
func FuzzDecodeBlockNeverPanics(f *testing.F) {
	f.Add(EncodeBlock([]Entry{{Namespace: 1, Path: "a.txt"}}))
	f.Add([]byte{0xff})
	f.Fuzz(func(t *testing.T, in []byte) {
