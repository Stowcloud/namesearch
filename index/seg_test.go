...
func TestAppendAndReadRecordsRoundTrip(t *testing.T) {
	...
	want := [][]Entry{
		...
		{{Namespace: 1, Path: "b.txt"}, {Namespace: 2, Path: "c.txt"}},
	}
	for i, e := range want {
...
func TestPayloadTakesTheSmallerOfRawAndCompressed(t *testing.T) {
	small, err := EncodePayload(1, []Entry{{Namespace: 1, Path: "a"}})
	if err != nil {
		t.Fatalf("EncodePayload: %v", err)
	}
	if small[0] != codecRaw {
	...
	var many []Entry
	for i := range 500 {
		many = append(many, Entry{Namespace: 1, Path: "a-very-repetitive-directory-name/file.txt" + string(rune(i))})
	}
...
func FuzzReadRecordsNeverPanics(f *testing.F) {
	payload, err := EncodePayload(1, []Entry{{Namespace: 1, Path: "a.txt"}})
	if err != nil {
