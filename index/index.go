...
// happened but whose directory sync failed. The index must stop answering until
// a rebuild reconciles whichever version is present after reopening.
type publicationUncertainError struct{ err error }

func (e *publicationUncertainError) Error() string {
	return "index: publication uncertain: " + e.err.Error()
}
...
func (ix *NameIndex) Query(needle []byte, limit int) (Result, error) {
	...
	if ix.base != nil {
		...
		for _, bid := range candidates {
			...
			for _, e := range entries {
				...
				if ix.tombstonedLocked(e.Namespace, e.Path, 0) {
					continue
				}
				k := tombKey{namespace: e.Namespace, path: e.Path}
				if !seen[k] {
					seen[k] = true
					hits = append(hits, makeHit(e.Namespace, e.Path, folded))
				}
			}
			if len(hits) == before {
	...
	for _, d := range ix.delta {
		...
		if ix.tombstonedLocked(d.namespace, d.path, d.seq) {
			continue
		}
		k := tombKey{namespace: d.namespace, path: d.path}
		if !seen[k] {
			seen[k] = true
			hits = append(hits, makeHit(d.namespace, d.path, folded))
		}
	}
...
func (ix *NameIndex) ChildrenOf(namespace Namespace, dir string) ([]string, error) {
	ix.mu.RLock()
	defer ix.mu.RUnlock()
	...
	keep := func(path string, seq uint64) {
		...
		if ix.tombstonedLocked(namespace, path, seq) {
			return
		}
		out[path] = true
	}
	...
	if ix.base != nil {
		if err := ix.base.EachUnder(namespace, dir, func(e Entry) error {
			keep(e.Path, 0)
			return nil
		}); err != nil {
			return nil, err
...
func (ix *NameIndex) tombstonedLocked(namespace Namespace, path string, seq uint64) bool {
	at, ok := ix.tomb[tombKey{namespace: namespace, path: path}]
...
func makeHit(namespace Namespace, path string, folded []byte) Hit {
	name := path
	if i := strings.LastIndexByte(path, '/'); i >= 0 {
		name = path[i+1:]
	}
	return Hit{Namespace: namespace, Path: path, Name: name, Score: search.Score(search.RankInput{
		NameFolded: search.FoldString(name), Needle: folded, Path: path,
	})}
}
...
func (s mergeSnapshot) build(ctx context.Context, gate func() bool, cfg Config) ([]byte, error) {
	tombstoned := func(namespace Namespace, path string, seq uint64) bool {
		at, ok := s.tomb[tombKey{namespace: namespace, path: path}]
		return ok && at >= seq
	}
	var entries []Entry
	if s.base != nil {
		if err := s.base.EachEntry(func(e Entry) error {
			...
			if tombstoned(e.Namespace, e.Path, 0) {
				return nil
			}
	...
	for _, e := range entries {
		seen[tombKey{namespace: e.Namespace, path: e.Path}] = true
	}
	for _, d := range s.delta {
		if err := ctx.Err(); err != nil {
			return nil, err
		}
		if tombstoned(d.namespace, d.path, d.seq) {
			continue
		}
		k := tombKey{namespace: d.namespace, path: d.path}
		if seen[k] {
		...
		seen[k] = true
		entries = append(entries, Entry{Namespace: d.namespace, Path: d.path})
	}
...
func (ix *NameIndex) publish(snap mergeSnapshot, buf []byte) error {
	...
	// leaving a half-written index. If the rename happened but the directory
	// sync failed, the cache must stop answering until a rebuild reconciles the
	// on-disk state: the destination may be either base.
	basePath := filepath.Join(ix.dir, baseName)
	res, werr := fsatomic.ReplaceFileDurable(basePath, 0o600, func(f *os.File) error {
		_, err := f.Write(buf)
		return err
	})
	if werr != nil {
...
func rewriteTombstones(dir string, survivors map[tombKey]uint64) (int64, error) {
	...
	for k, at := range survivors {
		bySeq[at] = append(bySeq[at], Entry{Namespace: k.namespace, Path: k.path})
	}
	...
	res, werr := fsatomic.ReplaceFileDurable(tombPath, 0o600, func(f *os.File) error {
		_, err := f.Write(body)
		return err
	})
	if werr != nil {
		if res.Outcome == fsatomic.PublicationUncertain {
			return 0, &publicationUncertainError{err: werr}
		}
