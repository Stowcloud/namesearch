...
import (
	...
	"github.com/stowcloud/namesearch/internal/task"
)
...
func TestAppendIsVisibleToTheNextQuery(t *testing.T) {
	...
	if err := ix.Append([]Entry{{Namespace: 1, Path: "a/report.pdf"}}); err != nil {
...
func TestATombstoneHidesANameBeforeTheMerge(t *testing.T) {
	...
	if err := ix.Append([]Entry{
		{Namespace: 1, Path: "keep.txt"},
		{Namespace: 1, Path: "gone.txt"},
	}); err != nil {
		t.Fatalf("Append: %v", err)
	}
	if err := ix.Tombstone([]Entry{{Namespace: 1, Path: "gone.txt"}}); err != nil {
...
func TestDeleteThenRecreateEndsUpLive(t *testing.T) {
	ix := openIndex(t, t.TempDir())
	entry := []Entry{{Namespace: 1, Path: "again.txt"}}
...
func TestAShortQueryFallsBack(t *testing.T) {
	ix := openIndex(t, t.TempDir())
	if err := ix.Append([]Entry{{Namespace: 1, Path: "ab.txt"}}); err != nil {
		t.Fatalf("Append: %v", err)
	}
...
func TestAnIncompleteIndexDeclinesEveryQuery(t *testing.T) {
	ix := openIndex(t, t.TempDir())
	if err := ix.Append([]Entry{{Namespace: 1, Path: "present.txt"}}); err != nil {
		t.Fatalf("Append: %v", err)
	}
...
func TestAllTrigramsPrunedFallsBack(t *testing.T) {
	...
	for i := range MinBlocksForPrune * 2 {
		corpus = append(corpus, Entry{Namespace: 1, Path: string(rune('a'+i)) + "-common"})
	}
...
func TestMergeCollapsesTheOverlayAndKeepsTheNames(t *testing.T) {
	...
	var corpus []Entry
	for i := range 100 {
		corpus = append(corpus, Entry{Namespace: 1, Path: filepath.Join("d", "file-"+string(rune('a'+i%26))+string(rune('a'+i/26))+".txt")})
	}
	if err := ix.Append(corpus); err != nil {
		t.Fatalf("Append: %v", err)
	}
...
func TestMergeKeepsATombstoneNewerThanTheNewBase(t *testing.T) {
	ix := openIndex(t, t.TempDir())
	if err := ix.Append([]Entry{{Namespace: 1, Path: "doomed.txt"}, {Namespace: 1, Path: "safe.txt"}}); err != nil {
		t.Fatalf("Append: %v", err)
	}
	if err := ix.Merge(t.Context(), nil); err != nil {
		t.Fatalf("first merge: %v", err)
	}
	if err := ix.Tombstone([]Entry{{Namespace: 1, Path: "doomed.txt"}}); err != nil {
		t.Fatalf("Tombstone: %v", err)
	}
	if err := ix.Merge(t.Context(), nil); err != nil {
...
func TestARefusedMergeChangesNothing(t *testing.T) {
	ix := openIndex(t, t.TempDir())
	if err := ix.Append([]Entry{{Namespace: 1, Path: "a.txt"}}); err != nil {
		t.Fatalf("Append: %v", err)
	}
	before := ix.Stats()
...
func TestQueriesDuringAMergeSeeAConsistentSet(t *testing.T) {
	...
	var corpus []Entry
	for i := range 400 {
		corpus = append(corpus, Entry{Namespace: 1, Path: "dir/report-" + string(rune('a'+i%26)) + string(rune('a'+i/26)) + ".txt"})
	}
	if err := ix.Append(corpus); err != nil {
		t.Fatalf("Append: %v", err)
	}
...
func TestConcurrentMergesDoNotLoseWrites(t *testing.T) {
	...
	var corpus []Entry
	for i := range 200 {
		corpus = append(corpus, Entry{Namespace: 1, Path: "f-" + string(rune('a'+i%26)) + string(rune('a'+i/26)) + ".txt"})
	}
	if err := ix.Append(corpus); err != nil {
		t.Fatalf("Append: %v", err)
	}
...
func TestReopenRecoversTheOverlay(t *testing.T) {
	dir := t.TempDir()
	ix := openIndex(t, dir)
	if err := ix.Append([]Entry{{Namespace: 1, Path: "kept.txt"}}); err != nil {
		t.Fatalf("Append: %v", err)
	}
	if err := ix.Tombstone([]Entry{{Namespace: 1, Path: "removed.txt"}}); err != nil {
		t.Fatalf("Tombstone: %v", err)
	}
...
func TestChildrenOfIsDirectChildrenWithTheOverlayApplied(t *testing.T) {
	ix := openIndex(t, t.TempDir())
	if err := ix.Append([]Entry{
		{Namespace: 1, Path: "d/a.txt"},
		{Namespace: 1, Path: "d/b.txt"},
		{Namespace: 1, Path: "d/deep/c.txt"},
		{Namespace: 1, Path: "other.txt"},
		{Namespace: 2, Path: "d/elsewhere.txt"},
	}); err != nil {
		t.Fatalf("Append: %v", err)
	}
	...
	if terr := ix.Tombstone([]Entry{{Namespace: 1, Path: "d/a.txt"}}); terr != nil {
