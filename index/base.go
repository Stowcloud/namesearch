...
import (
	...
	"github.com/stowcloud/namesearch/internal/num"
)
...
// Namespace identifies the indexed namespace while retaining the uint32 on-disk encoding.
...
func EncodeBlock(entries []Entry) []byte {
	...
	for _, e := range entries {
		out = search.PutVarint(out, uint64(e.Namespace))
...
func DecodeBlock(payload []byte) ([]Entry, error) {
	...
	pos := 0
	for pos < len(payload) {
		namespace, next, err := search.Varint(payload, pos)
		if err != nil {
			return nil, fmt.Errorf("%w: a truncated namespace id in a block", ErrCorrupt)
		}
		pos = next
		...
		id, serr := num.Narrow[Namespace](namespace)
		if serr != nil {
			return nil, fmt.Errorf("%w: a namespace id that cannot be one", ErrCorrupt)
		}
		end := pos + n
		out = append(out, Entry{Namespace: id, Path: string(payload[pos:end])})
		pos = end
	}
	return out, nil
}
...
// entryLess orders two entries as TreeOrder wrote them: by namespace first, then by
// path in tree order.
func entryLess(aNamespace Namespace, aPath string, bNamespace Namespace, bPath string) bool {
...
func (s *BaseSegment) EachUnder(namespace Namespace, dir string, fn func(Entry) error) error {
	if s.BlockCount == 0 {
		return nil
	}
	...
	for lo < hi {
		...
		if entryLess(first.Namespace, first.Path, namespace, dir) {
			lo = mid + 1
			continue
		}
		hi = mid
