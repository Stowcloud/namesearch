...
func EncodePayload(seq uint64, entries []Entry) ([]byte, error) {
	...
	for _, e := range entries {
		body = search.PutVarint(body, uint64(e.Namespace))
...
func DecodePayload(payload []byte) (uint64, []Entry, error) {
	...
	for range count {
		namespace, next, serr := search.Varint(body, pos)
		if serr != nil {
			return 0, nil, fmt.Errorf("%w: a truncated namespace id in a record", ErrCorrupt)
		}
